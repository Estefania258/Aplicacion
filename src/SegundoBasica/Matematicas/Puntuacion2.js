...
const Puntuacion2 = () => {
  ...
  useEffect(() => {
    ...
    Object.keys(localStorage).forEach((clave) => {
      if (clave.startsWith('puntuacionp_P')) {
        const valor = JSON.parse(localStorage.getItem(clave));

        let pregunta;
        try {
          pregunta = parseInt(clave.replace('puntuacionp_P', ''));
        } catch (error) {
          console.error("Error al procesar la clave: ", error);
          pregunta = null; // O el valor por defecto que desees
        }

        // Aseguramos que 'respuestasIncorrectas' sea un arreglo
        const respuestasIncorrectas = Array.isArray(valor.respuestasIncorrectas) ? valor.respuestasIncorrectas : [];
